...
This project is built from scratch without spring-boot or heavy frameworks to demonstrate core distributed systems concepts, Kafka internals, and raw JVM performance.
...
3. **Aggregator**: Consumes from the `results` topic, updates global stats (running throughput, longest chain record), and prints a live dashboard.
4. **Kafka Broker**: Acts as the message backbone. Configured to automatically split work into **3 partitions** to enable horizontal scaling and consumer-group rebalancing.
...
If you have `tmux` installed on your machine, you can run the entire system (Kafka startup, compilation, and spawning of 2 workers + 1 aggregator + 1 coordinator in a split-screen layout) with a single command:
...
## 🎓 Core Kafka Concepts Mastered

* **Partitions & Parallelism**: A topic is divided into partitions. Kafka distributes these partitions across consumers in a consumer group. If a topic has 3 partitions, up to 3 workers can process messages concurrently.
...
* **Rebalancing & Fault Tolerance**: If a worker crashes mid-chunk, Kafka detects the heartbeat failure, triggers a rebalance, and reassigns its partition to a surviving worker.
* **Offsets & At-Least-Once Delivery**: Workers disable auto-commit and call `consumer.commitSync()` only *after* completing a chunk. If a worker dies mid-chunk, the next worker re-reads the chunk from the last committed offset, ensuring no work is lost.
